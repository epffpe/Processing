...
//  println(x52.getX());
  ...
  text("Presion 'x' para autotrim OFF", width/2, 40, 0);
  float rumbo = Headingref;
  if (rumbo < 0) rumbo +=360;
  text("Rumbo: " + rumbo, width/2, 60, 0);
  text("Altura: " + Href, width/2, 75, 0);
  text("Speed: " + Speedref, width/2, 90, 0);
  if(autotrim) text("Autotrim ON", width/2, height/2 +40, 0);
  if(autopilotMode) text("AutoPilot ON", width/2, height/2 +55, 0);
...
//  }
//  byte[] msg = xplane.flightControl(elev, airln, rudder);
  byte[] msg = xplane.flightSpeedControl(elev, airln, rudder, speedCnt);
  udp.send( msg, ip, port );
