...
void receive( byte[] data, String ip, int port ) {	// <-- extended handler
  ...
  float div_Q = 0.3 + pow(velocidad/200,2);
  float div_P = 0.5 + pow(velocidad/200,2);
  float div_R = 0.5 + pow(velocidad/250,2);
  float gy = xplane.Q()/div_Q;
  float gx = xplane.P()/div_P;
  float gz = xplane.R()/div_R;
    
//  float gy = (xplane.Q() + 3 * gy_prev)/4;
//  float gx = (xplane.P() + 5 * gx_prev)/6;
//  float gz = (xplane.R() + 3 * gz_prev)/4;
