...
public class Magnetometer_calibration extends PApplet {
...
public void setupRazor() {
  ...
  */
  serial.write("#magcal\r"); //Entrando en modo calibracion
  delay(1000);
  // Synch with Razor
...
public void skipBytes(Serial s, int numBytes) {
  for (int i = 0; i < numBytes; i++) {
    s.read();
  }  
}

public boolean readSynch(Serial serial) {
  // Wait until enough bytes are available
...
public void draw() {
  ...
  pushMatrix(); {
    ...
    if (magnetomIndex < NUM_MAGN_SAMPLES && serial.available() >= 16) {
      ...
      while (serial.available() >= 16) {
        ...
        if ( readSynch(serial) ){
          ...
          //println("Trama recibida " + magnetom[magnetomIndex][0] + "," + magnetom[magnetomIndex][1] + "," + magnetom[magnetomIndex][2]);
        }
        // Skip gyro data
        //skipBytes(serial, 12);
      }
...
public void outputCalibration() {
  ...
  */
  ...
  SimpleMatrix radii = new SimpleMatrix(new double[][]
    {{Math.sqrt(1.0f / evd.getEigenvalue(0).getReal()),
      Math.sqrt(1.0f / evd.getEigenvalue(1).getReal()),
      Math.sqrt(1.0f / evd.getEigenvalue(2).getReal())}});
  ...
  */
  ...
  println("\n");
  /*
  int sincronizacion = 0xAAAAAAAA;
  serial.write(sincronizacion);
  serial.write( center.get(0));
  serial.write( center.get(1));
  serial.write( center.get(2));
  serial.write( comp.get(0));
  serial.write( comp.get(1));
  serial.write( comp.get(2));
  serial.write( comp.get(3));
  serial.write( comp.get(4));
  serial.write( comp.get(5));
  serial.write( comp.get(6));
  serial.write( comp.get(7));
  serial.write( comp.get(8));
  */
}
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "Magnetometer_calibration" });
  }
}
